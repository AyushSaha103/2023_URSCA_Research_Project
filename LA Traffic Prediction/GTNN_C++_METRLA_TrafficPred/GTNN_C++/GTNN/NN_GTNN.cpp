...
float NN_GTNN::predict(float*& h1, float*& h2) {
...
void NN_GTNN::backward(float*& input1, float*& input2, float& dcda_final, float scale_lrate) {
    ...
    while (ptr != nullptr) {
        ...
        NN_GTNN::deriv_activmap[ptr->activation]\
...
float**& NN_GTNN::get_layer0_dcda() { return layers[0]->dcda; }


// normal backprop., but we're passed final layer dcda
void NN_GTNN::backward_using_derivative(float**& dcda_final) {
    NNlayer* ptr = layers[numLayers - 1];
...
float NN_GTNN::backward(float**& y_true) {
